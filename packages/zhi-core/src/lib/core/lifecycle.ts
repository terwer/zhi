...
import { createCoreLogger, SystemImport } from "../utils/index.js"
...
class Lifecycle {
  ...
  constructor() {
    this.logger = createCoreLogger("lifecycle")
  }
  ...
  public async load() {
    ...
    const zhiSchemaPath = this.siyuanDevice.getZhiThemeImportJsPath(this.ZHI_JSON_SCHEMA)
    const { default: zhiSchema } = await import(zhiSchemaPath)
    const zhiJsonPath = this.siyuanDevice.getZhiThemeImportJsPath(this.ZHI_JSON)
