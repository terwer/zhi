...
import { handleFromMain, removeEventListeners, sendToMain } from "~/src/events/enentHandler"
...
class SyPicgo {
  ...
  constructor(configPath: string) {
        ...
        if (autoRename) {
          await Promise.all(
            ctx.output.map(async (item, index) => {
              const fileName = dayjs().add(index, "ms").format("YYYYMMDDHHmmSSS") + item.extname
    ...
    // 事件注册
    this.ipcMethods = {
      handleEvent: (channel: string, args?: any) => {
