...
// 重构版暂时无法运行，沿用旧版
// import { BrowserUtil, PublishHook } from "@terwer/publisher-hook"
//
// // 统一的初始化入口
// ;(async () => {
//   console.warn(
//     "从 0.6.8+ 开始，siyuanhook.js 将作为挂件统一的 hook 入口。" +
//       "当前 siyuanhook.js 源码已迁移至：https://github.com/terwer/zhi/tree/main/libs/widget-siyuan-hook 统一管理。"
//   )
//   try {
...
const getSiyuanNewWinDataDir = () => {
  return (window as any).terwer.dataDir ?? "/notfound"
}
...
const getSiyuanDataDir = () => {
  const syWin = getSiyuanWindow() as any
 ...
 * @since 0.7.0
 */
const requireLib = (entryName: string, libpath: string, alias: string) => {
